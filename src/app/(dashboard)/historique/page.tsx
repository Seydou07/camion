  ...
  useEffect(() => {
    const loadActivities = async () => {
      try {
        ...
        // Map carburant
        (resCarb || []).forEach((dossier: any) => {
          (dossier.mouvements || []).forEach((mouvement: any) => {
            const chauffeurLabel = dossier.chauffeur
              ? `${dossier.chauffeur.prenom || ""} ${dossier.chauffeur.nom}`.trim()
              : "le chauffeur";
            const recuCount = mouvement.recus?.length || 0;
            const stationLabel = mouvement.stationService || "inconnue";
            const kmLabel =
              typeof mouvement.kilometrage === "number"
                ? ` au kilométrage ${mouvement.kilometrage.toLocaleString()} km`
                : "";
            const litresLabel =
              typeof mouvement.litres === "number"
                ? `${mouvement.litres.toLocaleString("fr-FR")} L`
                : "Montant carburant";
            const recuLabel =
              recuCount > 0 ? ` ${recuCount} reçu(x) joint(s).` : " Aucun reçu joint.";

            formatted.push({
              id: `fuel-${dossier.id}-${mouvement.id}`,
              type: "carburant",
              title: `Ticket carburant - ${dossier.camion?.immatriculation || "Véhicule"}`,
              description: `${litresLabel} ajoutés à la station "${stationLabel}"${kmLabel} par ${chauffeurLabel}. Coût : ${formatMontant(mouvement.montant)}.${recuLabel}`,
              dateObj: new Date(mouvement.dateOperation),
              icon: (
